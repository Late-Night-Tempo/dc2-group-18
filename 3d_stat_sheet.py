...
# question_list = ["NQ135BD;377", "NQ21;55", "Q61;97", "Q62A;98", "Q62TI;106",]
# question2 = ["Q62B;99", "Q62C;100", "Q62H;105", "Q62E;102", "Q60;96"]
question_list = ["C2;3", "Q62B;99", "Q62C;100", "Q62H;105", "Q62E;102", "Q60;96", "MONTH_YEAR", "MONTH", "YEAR"]
...
df_pivot = trust_merge.pivot_table(index=['YEAR', 'Area Name'], columns='Measure', values='Proportion',
...
all_global_cols = ["YEAR", "LLW", "Inac_Perc_Fem", "Inac_Perc_Mal", "Week_Inc", "Q62B;99", "Q62C;100", "Q62H;105",
                   "Q62E;102", "Q60;96", target_category]
input_and_q_cols = ["YEAR", "LLW", "Inac_Perc_Fem", "Inac_Perc_Mal", "Week_Inc", "Q62B;99", "Q62C;100", "Q62H;105",
                    "Q62E;102", "Q60;96"]
...
# Set predictions and results for model 1
...
print("MSE:", mean_squared_error(X_test["Predicted"], X_test["Actual"]))
print("MAE:", mean_absolute_error(X_test["Predicted"], X_test["Actual"]))
print()
...
## BEGIN MODEL 2 SETTING

# Encode categorical variables using OrdinalEncoder
ordinal_encoder = OrdinalEncoder()
categorical_columns = ["Q62B;99", "Q62C;100", "Q62H;105", "Q62E;102", "Q60;96"]
pas_logistic_question_filtered[categorical_columns] = ordinal_encoder.fit_transform(
    pas_logistic_question_filtered[categorical_columns].astype(str))
pas_logistic_question_filtered["Area Name"] = pas_logistic["Area Name"]
...
# Add the test features to the results for further analysis
results_df2 = pd.concat([results_df2, X2_test.reset_index(drop=True)], axis=1)


X2_test["Actual"] = y2_test
...
print("MSE:", mean_squared_error(X2_test["Predicted"], X2_test["Actual"]))
print("MAE:", mean_absolute_error(X2_test["Predicted"], X2_test["Actual"]))
...
sns.scatterplot(x=results_df['Actual'], y=results_df['Predicted'],  s=50, alpha=0.4)
plt.plot([results_df['Actual'].min(), results_df['Actual'].max()],
         [results_df['Actual'].min(), results_df['Actual'].max()],
         color='chocolate', linestyle='--', linewidth=2)
plt.xlabel('Actual "Trust in MPS" Proportion', weight='bold', size=16)
plt.ylabel('Predicted "Trust in MPS" Proportion', weight='bold', size=16)
plt.title(f'Trust in the MPS as a Result of Socioeconomic Factors:\n Actual vs. Predicted', weight='bold', size=20)
# Title - bold and size 20
# Axis title - bold and size 16
# Axis ticks - bold and size 16
# Legend - title = 16 text = 14
plt.xticks(fontsize=16, weight='bold')
plt.yticks(fontsize=16, weight='bold')
plt.legend(labels=["Results from Dataset", "Ideal Baseline"], fontsize=14, title_fontsize=16)
plt.tight_layout()
plt.show()
...
plt.figure(figsize=(10, 10))
sns.scatterplot(x=results_df['Predicted'], y=results_df['Residuals'],  s=50, alpha=0.4)
plt.axhline(0, color='chocolate', linestyle='--', linewidth=2)
plt.xlabel('Predicted "Trust in MPS" Proportion' , weight='bold', size=16)
plt.ylabel('Residuals of "Trust in MPS" Proportion', weight='bold', size=16)
plt.title('Trust in the MPS as a Result of Socioeconomic Factors:\n Residuals vs. Predicted', weight='bold', size=20)
plt.legend(labels=["Results from Dataset", "Ideal Baseline"], fontsize=14, title_fontsize=16)
plt.xticks(fontsize=16, weight='bold')
plt.yticks(fontsize=16, weight='bold')
plt.tight_layout()
...
plt.figure(figsize=(10, 10))
sns.scatterplot(x=results_df2['Actual'], y=results_df2['Predicted'],  s=50, alpha=0.4)
plt.plot([results_df2['Actual'].min(), results_df2['Actual'].max()],
         [results_df2['Actual'].min(), results_df2['Actual'].max()],
         color='chocolate', linestyle='--', linewidth=2)
plt.legend(labels=["Results from Dataset", "Ideal Baseline"], fontsize=14, title_fontsize=16)
plt.xlabel('Actual "Trust in MPS" Proportion', weight='bold', size=16)
plt.ylabel('Predicted "Trust in MPS" Proportion', weight='bold', size=16)
plt.title('Trust in the MPS as a Result of Socioeconomic Factors\n and Five PAS Survey Questions: Actual vs. Predicted', weight='bold', size=20)
plt.xticks(fontsize=16, weight='bold')
plt.yticks(fontsize=16, weight='bold')
plt.tight_layout()
plt.show()
...
# Calculate residuals
results_df2['Residuals'] = results_df2['Actual'] - results_df2['Predicted']


plt.figure(figsize=(10, 10))
sns.scatterplot(x=results_df2['Predicted'], y=results_df2['Residuals'],  s=50, alpha=0.4)
plt.axhline(0, color='chocolate', linestyle='--', linewidth=2)
plt.xlabel('Predicted "Trust in MPS" Proportion', weight='bold', size=16)
plt.ylabel('Residuals "Trust in MPS" Proportion', weight='bold', size=16)
plt.title('Trust in the MPS as a Result of Socioeconomic Factors\n and Five PAS Survey Questions: Residuals vs. Predicted', weight='bold', size=20)
plt.legend(labels=["Results from Dataset", "Ideal Baseline"], fontsize=14, title_fontsize=16)
plt.xticks(fontsize=16, weight='bold')
plt.yticks(fontsize=16, weight='bold')
plt.tight_layout()
...
##### PER BOROUGH #######
#
# for boroughs in pas_logistic_question_filtered["Area Name"].unique()[3:6]:
#     grouped_df = pas_logistic_question_filtered[pas_logistic_question_filtered["Area Name"] == boroughs].dropna(
#         subset=[target_category])
#
#     # Set X and y
#     input_and_q_cols = [col for col in all_global_cols if col != target_category]
#     X2 = grouped_df[input_and_q_cols]
#     y2 = grouped_df[target_category]
#
#     # Train-test split
#     # use random_state to act as a seed, if needed
#     # test_size defaults to .25, so 25% of data is used as test data
...
#     X2_test["Predicted"] = y2_pred
#
#     # print(X2_test["Actual"].unique())
#     # print(X2_test["Predicted"].unique())
#     # print(len(X2_train), len(X2_test), len(y2_train), len(y2_test))
#
#     plt.figure(figsize=(10, 10))
#     sns.scatterplot(x=X2_test['Actual'], y=X2_test['Predicted'])
#     plt.plot([X2_test['Actual'].min(), X2_test['Actual'].max()],
#              [X2_test['Actual'].min(), X2_test['Actual'].max()],
#              color='red', linestyle='--')
#     plt.xlabel('Actual')
#     plt.ylabel('Predicted')
#     plt.title(f'Model {boroughs} Actual vs. Predicted')
#     #plt.show()
#
#     # Calculate residuals
#     X2_test['Residuals'] = X2_test['Actual'] - X2_test['Predicted']
#
#     # Plotting
#     plt.figure(figsize=(10, 6))
#     sns.scatterplot(x=X2_test['Predicted'], y=X2_test['Residuals'])
#     plt.axhline(0, color='red', linestyle='--')
#     plt.xlabel('Predicted')
#     plt.ylabel('Residuals')
#     plt.title(f'Residuals {boroughs}  vs. Predicted')
#     #plt.show()
#
#     # Print details of the linear regression model
#     print(f"Per borough {boroughs} ADDED REGRESSION")
#     print(f"Category: {input_and_q_cols}")
#     print(f"Target Category: {target_category}")
#     print(f"The category with the largest absolute value coefficient is: {largest_abs_coefficient2}")
#     print(f"The coefficient value is: {largest_abs_value2}")
